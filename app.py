...
def load_transactions():
    try:
        response = requests.get(f'{DATA_STORE_URL}/transactions?store=gotatopup', timeout=5)
...
def save_transactions(transactions):
    try:
        response = requests.post(f'{DATA_STORE_URL}/transactions?store=gotatopup', 
...
def add_transaction_to_store(transaction_data, status):
    try:
        response = requests.post(f'{DATA_STORE_URL}/transactions?store=gotatopup/add', 
                               json={
                                   'status': status,
...
def load_packages():
    try:
        response = requests.get(f'{DATA_STORE_URL}/packages?store=gotatopup', timeout=5)
        response.raise_for_status()
        return response.json()
    except requests.RequestException:
...
def update_package():
    try:
        ...
        # Update via API
        response = requests.post(f'{DATA_STORE_URL}/packages/update?store=gotatopup', 
                               json={
                                   'game_type': game_type,
